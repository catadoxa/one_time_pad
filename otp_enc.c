...
int main(int argc, char *argv[]) {
	...
	//unnamed parameters: argv[1] = plaintext, argv[2] = key, argv[3] = port
	...
	if((text_len = read_text(text, argv[1], MAX_SIZE)) < 0)
		error("CLIENT: Failed to read plaintext", 1);
	...
	if(!(send_message(connectionFD, text, text_len) > 0))
		error("CLIENT: Failed to send text", 1);
	...
	if(!(receive_message(connectionFD, text, text_len, MSG_WAITALL)))
		error("CLIENT: Failed to receive encoded message", 1);
	
	//output text received from server
	if(fprintf(stdout, "%s\n", text) < 0)
		error("CLIENT: Failed to write encoded message to file", 1);
