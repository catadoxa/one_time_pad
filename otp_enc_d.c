...
char code_char(char plain, char pad);
...
//child process communicates with client and performs encoding
void run_child(int connectionFD) {
	...
	if(!(receive_message(connectionFD, text, text_len, MSG_WAITALL)))
		error("SERVER: Failed to receive text", 1);
	...
	//send back coded message
	if(!(send_message(connectionFD, text, text_len) > 0))
		error("SERVER: Failed to send encoded message", 1);
...
//codes a character using pad from key
char code_char(char plain, char pad) {
	
	//convert chars to numbers from 0 to 26
	if(plain == ' ')
		plain = 26;
	else if(plain <= 'Z' && plain >= 'A')
		plain -= 'A';
	else
		error("SERVER: Text contains bad characters", 1);;
	...
	//add and take mod 27 to get value from 0 to 26
	plain = (plain + pad) % 27;
...
int handshake(int connectionFD, int *text_len, int *key_len) {
	...
	//fprintf(stdout, "pass: %s, text len: %d, key_len: %d\n", pass, *text_len, *key_len);
